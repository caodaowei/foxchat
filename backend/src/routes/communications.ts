...
// 获取沟通记录列表
export async function getCommunications(_request: FastifyRequest, reply: FastifyReply) {
...
// 获取我的消息
export async function getMyCommunications(request: FastifyRequest, reply: FastifyReply) {
  const user = request.user as { id: string };
  
  const myCommunications = await db.query.communications.findMany({
    where: or(
      eq(communications.receiverId, user.id),
...
// 创建沟通记录
export async function createCommunication(request: FastifyRequest, reply: FastifyReply) {
...
// 标记已读
export async function markAsRead(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
  const user = request.user as { id: string };
...
// 删除沟通记录
export async function deleteCommunication(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
  const user = request.user as { id: string };
